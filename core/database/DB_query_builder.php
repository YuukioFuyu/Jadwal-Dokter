...
abstract class RAST_DB_query_builder extends RAST_DB_driver {
	...
	protected function _compile_wh($qb_key)
	{
		if (count($this->$qb_key) > 0)
		{
			for ($i = 0, $c = count($this->$qb_key); $i < $c; $i++)
			{
				...
				for ($rast = 0, $cc = count($conditions); $rast < $cc; $rast++)
				{
					if (($op = $this->_get_operator($conditions[$rast])) === FALSE
						OR ! preg_match('/^(\(?)(.*)('.preg_quote($op, '/').')\s*(.*(?<!\)))?(\)?)$/i', $conditions[$rast], $matches))
					{
						continue;
					}
					...
					$conditions[$rast] = $matches[1].$this->protect_identifiers(trim($matches[2]))
