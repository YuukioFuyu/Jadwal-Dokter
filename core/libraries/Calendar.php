...
class RAST_Calendar {
	...
	public function initialize($config = array())
	{
		...
		{
			$this->next_prev_url = $this->RAST->config->site_url($this->RAST->router->class.'/'.$this->RAST->router->method);
		}
	...
	public function get_month_name($month)
	{
		...
		return ($this->RAST->lang->line($month_names[$month]) === FALSE)
			? ucfirst(substr($month_names[$month], 4))
			: $this->RAST->lang->line($month_names[$month]);
	}
	...
	public function get_day_names($day_type = '')
	{
		...
		{
			$days[] = ($this->RAST->lang->line('cal_'.$day_names[$i]) === FALSE) ? ucfirst($day_names[$i]) : $this->RAST->lang->line('cal_'.$day_names[$i]);
		}
