...
class RAST_Xmlrpc {
	...
	public function __construct($config = array())
	{
		...
		log_message('info', 'RAST XML-RPC Class Initialized');
	}
...
class XML_RPC_Client extends RAST_Xmlrpc
{
	public $path			= '';
	public $server			= '';
	public $port			= 80;
	...
	public function decode($array = NULL)
	{
		...
		if (is_array($array))
		{
			while (list($key) = each($array))
			{
				...
				{
					$array[$key] = $RAST->security->xss_clean($array[$key]);
				}
			}
		...
		elseif ($this->xss_clean)
		{
			$result = $RAST->security->xss_clean($result);
		}
...
class XML_RPC_Message extends RAST_Xmlrpc
{
	public $payload;
	public $method_name;
	public $params		= array();
	...
	public function output_parameters(array $array = array())
	{
		...
		for ($i = 0, $c = count($this->params); $i < $c; $i++)
		{
			...
			{
				$parameters[] = ($this->xss_clean) ? $RAST->security->xss_clean($a_param) : $a_param;
			}
		}
...
class XML_RPC_Values extends RAST_Xmlrpc
{
	public $me	= array();
	public $mytype	= 0;
