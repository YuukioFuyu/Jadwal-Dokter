...
class RAST_Upload {
	...
	public function __construct($config = array())
	{
		...
		$this->_RAST =& get_instance();

		log_message('info', 'RAST Upload Class Initialized');
	}
	...
	public function do_upload($field = 'userfile')
	{
		...
		$this->file_name = $this->_RAST->security->sanitize_filename($this->file_name);
	...
	public function do_xss_clean()
	{
		...
		return $this->_RAST->security->xss_clean($data, TRUE);
	}

	public function set_error($msg, $log_level = 'error')
	{
		$this->_RAST->lang->load('upload');

		is_array($msg) OR $msg = array($msg);
		foreach ($msg as $val)
		{
			$msg = ($this->_RAST->lang->line($val) === FALSE) ? $val : $this->_RAST->lang->line($val);
			$this->error_msg[] = $msg;
