...
class RAST_Image_lib {
	...
	public function __construct($props = array())
	{
		...
		log_message('info', 'RAST Image Lib Class Initialized');
	}
	...
	{
		$RAST =& get_instance();
		$RAST->lang->load('imglib');

		if (is_array($msg))
		{
			foreach ($msg as $val)
			{
				$msg = ($RAST->lang->line($val) === FALSE) ? $val : $RAST->lang->line($val);
				$this->error_msg[] = $msg;
		...
		{
			$msg = ($RAST->lang->line($msg) === FALSE) ? $msg : $RAST->lang->line($msg);
			$this->error_msg[] = $msg;
			log_message('error', $msg);
		}
	}
