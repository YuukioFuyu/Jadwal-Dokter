...
class RAST_Jquery extends RAST_Javascript {
	...
	public function __construct($params)
	{
		...
		log_message('info', 'RAST Jquery Class Initialized');
	}
	...
	protected function _updater($container = 'this', $controller = '', $options = '')
	{
		...
		$controller = (strpos('://', $controller) === FALSE) ? $controller : $this->RAST->config->site_url($controller);

		if ($this->RAST->config->item('javascript_ajax_img') === '')
		{
			$loading_notifier = 'Loading...';
		}
		else
		{
			$loading_notifier = '<img src="'.$this->RAST->config->slash_item('base_url').$this->RAST->config->item('javascript_ajax_img').'" alt="Loading" />';
		}
	...
	protected function _compile($view_var = 'script_foot', $script_tags = TRUE)
	{
		...
		$this->RAST->load->vars(array('library_src' => $external_scripts));
		...
		$this->RAST->load->vars(array($view_var => $output));
	}
