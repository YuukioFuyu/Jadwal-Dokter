...
class RAST_Javascript {
	...
	public function __construct($params = array())
	{
		...
		$this->RAST->load->library('Javascript/'.$js_library_driver, array('autoload' => $autoload));
		$this->js =& $this->RAST->$js_library_driver;

		log_message('info', 'RAST Javascript Class Initialized and loaded. Driver used: '.$js_library_driver);
	}
	...
	public function external($external_file = '', $relative = FALSE)
	{
		...
		elseif ($this->RAST->config->item('javascript_location') !== '')
		{
			$this->_javascript_location = $this->RAST->config->item('javascript_location');
		}
		...
		{
			$str = $this->_open_script($this->RAST->config->slash_item('base_url').$this->_javascript_location.$external_file);
		}
	...
	{
		return '<script type="text/javascript" charset="'.strtolower($this->RAST->config->item('charset')).'"'
			.($src === '' ? '>' : ' src="'.$src.'">');
	}
