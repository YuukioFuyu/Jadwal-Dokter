...
class RAST_Form_validation {
	...
	public function __construct($rules = array())
	{
		...
		$this->RAST->load->helper('form');

		log_message('info', 'RAST Form Validation Class Initialized');
	}

	public function set_rules($field, $label = '', $rules = array(), $errors = array())
	{
		if ($this->RAST->input->method() !== 'post' && empty($this->validation_data))
		{
			return $this;
		}
	...
	public function run($group = '')
	{
		...
		if (count($this->_field_data) === 0)
		{
			...
			if (empty($group))
			{
				$group = trim($this->RAST->uri->ruri_string(), '/');
				isset($this->_config_rules[$group]) OR $group = $this->RAST->router->class.'/'.$this->RAST->router->method;
			}
		...
		$this->RAST->lang->load('form_validation');
	...
	protected function _execute($row, $rules, $postdata = NULL, $cycles = 0)
	{
		...
		foreach ($rules as $rule)
		{
			...
			if ($callback OR $callable !== FALSE)
			{
				if ($callback)
				{
					if ( ! method_exists($this->RAST, $rule))
					{
					...
					{
						$result = $this->RAST->$rule($postdata, $param);
					}
				}
				else
				{
			...
			if ($result === FALSE)
			{
				if ( ! is_string($rule))
				{
					$line = $this->RAST->lang->line('form_validation_error_message_not_set').'(Anonymous function)';
				}
				else
				{
	...
	protected function _get_error_message($rule, $field)
	{
		...
		elseif (FALSE !== ($line = $this->RAST->lang->line('form_validation_'.$rule)))
		{
			return $line;
		}
		elseif (FALSE !== ($line = $this->RAST->lang->line($rule, FALSE)))
		{
			return $line;
		}

		return $this->RAST->lang->line('form_validation_error_message_not_set').'('.$rule.')';
	}

	protected function _translate_fieldname($fieldname)
	{
		if (sscanf($fieldname, 'lang:%s', $line) === 1 && FALSE === ($fieldname = $this->RAST->lang->line($line, FALSE)))
		{
			return $line;
		}
	...
	public function is_unique($str, $field)
	{
		...
		return isset($this->RAST->db)
			? ($this->RAST->db->limit(1)->get_where($table, array($field => $str))->num_rows() === 0)
			: FALSE;
	}
	...
	{
		return $this->RAST->input->valid_ip($ip, $which);
	}
	...
	{
		return $this->RAST->security->strip_image_tags($str);
	}
