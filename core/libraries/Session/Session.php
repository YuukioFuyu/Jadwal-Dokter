...
class RAST_Session {
	...
	public function __construct(array $params = array())
	{
		...
		$class = $this->_rast_load_classes($this->_driver);
		...
		{
			if ( ! isset($_SESSION['__rast_last_regenerate']))
			{
				$_SESSION['__rast_last_regenerate'] = time();
			}
			elseif ($_SESSION['__rast_last_regenerate'] < (time() - $regenerate_time))
			{
		...
		$this->_rast_init_vars();

		log_message('info', "RAST Session: Class initialized using '".$this->_driver."' driver.");
	}

	protected function _rast_load_classes($driver)
	{
		...
		if ( ! class_exists('RAST_Session_driver', FALSE))
		{
			require_once(
		...
		if ( ! class_exists('RAST_'.$class, FALSE))
		{
			...
			if ( ! class_exists('RAST_'.$class, FALSE) && ! class_exists($class, FALSE))
			{
	...
	protected function _rast_init_vars()
	{
		if ( ! empty($_SESSION['__rast_vars']))
		{
			$current_time = time();

			foreach ($_SESSION['__rast_vars'] as $key => &$value)
			{
				if ($value === 'new')
				{
					$_SESSION['__rast_vars'][$key] = 'old';
				}
				elseif ($value < $current_time)
				{
					unset($_SESSION[$key], $_SESSION['__rast_vars'][$key]);
				}
			}

			if (empty($_SESSION['__rast_vars']))
			{
				unset($_SESSION['__rast_vars']);
			}
		}
	...
	public function mark_as_flash($key)
	{
		if (is_array($key))
		{
			...
			$_SESSION['__rast_vars'] = isset($_SESSION['__rast_vars'])
				? array_merge($_SESSION['__rast_vars'], $new)
				: $new;
		...
		$_SESSION['__rast_vars'][$key] = 'new';
		return TRUE;
	}

	public function get_flash_keys()
	{
		if ( ! isset($_SESSION['__rast_vars']))
		{
			return array();
		}

		$keys = array();
		foreach (array_keys($_SESSION['__rast_vars']) as $key)
		{
			is_int($_SESSION['__rast_vars'][$key]) OR $keys[] = $key;
		}
	...
	public function unmark_flash($key)
	{
		...
		foreach ($key as $k)
		{
			if (isset($_SESSION['__rast_vars'][$k]) && ! is_int($_SESSION['__rast_vars'][$k]))
			{
				unset($_SESSION['__rast_vars'][$k]);
			}
		}
	...
	public function mark_as_temp($key, $ttl = 300)
	{
		...
		if (is_array($key))
		{
			...
			$_SESSION['__rast_vars'] = isset($_SESSION['__rast_vars'])
				? array_merge($_SESSION['__rast_vars'], $temp)
				: $temp;
	...
	public function get_temp_keys()
	{
		...
		foreach (array_keys($_SESSION['__rast_vars']) as $key)
		{
			is_int($_SESSION['__rast_vars'][$key]) && $keys[] = $key;
		}
	...
	public function flashdata($key = NULL)
	{
		if (isset($key))
		{
			return (isset($_SESSION['__rast_vars'], $_SESSION['__rast_vars'][$key], $_SESSION[$key]) && ! is_int($_SESSION['__rast_vars'][$key]))
				? $_SESSION[$key]
				: NULL;
		}
